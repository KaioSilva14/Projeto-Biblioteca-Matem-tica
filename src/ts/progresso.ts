// progresso.ts — acompanhamento local de progresso por tópico e ano.
export {};

// progresso.ts
// Leitura e escrita do progresso do aluno em localStorage.
// Nenhum dado pessoal é armazenado — apenas contadores por conteúdo.

import type { ProgressoConteudo, ProgressoArmazenado } from "./types.js";
...
function ehProgressoValido(valor: unknown): valor is ProgressoConteudo {
  if (typeof valor !== "object" || valor === null) {
    return false;
  }
  const registro = valor as Record<string, unknown>;
  return (
    typeof registro.conteudoId === "string" &&
 ...
 */
export function getAllProgress(): ProgressoArmazenado {
  try {
    ...
    const resultado: ProgressoArmazenado = {};
...
export function getProgress(conteudoId: string): ProgressoConteudo | null {
  const todos = getAllProgress();
  return todos[conteudoId] ?? null;
}

/** Salva (sobrescreve) o progresso de um conteúdo específico. */
export function saveProgress(progresso: ProgressoConteudo): void {
  try {
    const todos = getAllProgress();
...
export function updateProgress(
  conteudoId: string,
  resultado: { acertou: boolean; indiceAtividade: number; totalAtividades: number }
): ProgressoConteudo {
  ...
  const atualizado: ProgressoConteudo = {
    ...atual,
...
export function resetProgress(conteudoId: string): void {
  try {
    const todos = getAllProgress();
...
export function getProgressoEmAndamento(): ProgressoConteudo | null {
  const todos = getAllProgress();
