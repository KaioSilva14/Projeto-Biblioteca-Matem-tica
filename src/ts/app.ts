...
document.addEventListener("DOMContentLoaded", () => {
  ...
  switch (document.body.dataset.pagina) {
    ...
    case "404": void iniciarHome(); break;
  }
});
 ...
 */
function miolo(faixa: HTMLElement): HTMLElement {
  const existente = faixa.querySelector<HTMLElement>(":scope > .container");
  if (existente) {
    limpar(existente);
    return existente;
  }
...
function recado(
  ...
  titulo: string,
  motivo: string,
  saidas: { texto: string; href: string; primaria?: boolean }[] = []
): void {
  limpar(alvo);
...
async function iniciarAno(): Promise<void> {
  ...
  if (!ano) {
    return recado(
      alvo,
      "Esse ano não existe aqui",
      "A Biblioteca Matemática cobre do 6º ao 9º ano do Ensino Fundamental. O endereço que você abriu pede um ano fora dessa faixa — provavelmente um link digitado à mão.",
      [
...
async function iniciarCurso(): Promise<void> {
  ...
  if (!item) {
    return recado(
      alvo,
      "Essa matéria não existe",
      "Não há nenhuma matéria com esse endereço no site. Pode ser um link antigo ou um erro de digitação — nada foi removido daqui.",
      [
  ...
  if (!item.arquivo) {
    return recado(
      alvo,
      "Essa matéria ainda não está disponível",
      `${item.titulo} está no catálogo, mas ainda não foi escrita. As matérias são publicadas uma a uma, e cada uma só entra depois que todas as contas dela passam nos testes.`,
      [
  ...
  if (!curso) {
    return recado(
      alvo,
      "Não conseguimos carregar essa matéria",
      "O conteúdo existe, mas não chegou até aqui. Quase sempre é a conexão: recarregar a página costuma resolver.",
      [
...
async function iniciarLicao(): Promise<void> {
  ...
  if (!item?.arquivo || !licaoId) {
    return recado(
      alvo,
      "Não encontramos essa lição",
      "O endereço precisa dizer a matéria e a lição, e um dos dois está faltando ou não existe. Pode ser um link cortado pela metade ao ser copiado.",
      [
        { texto: "Ver as matérias por ano", href: "/#anos", primaria: true },
        { texto: "Como o site funciona", href: "/sobre.html" },
      ]
  ...
  if (!curso || !entrada) {
    return recado(
      alvo,
      "Essa lição não existe nesta matéria",
      curso
  ...
  if (!licao) {
    return recado(
      alvo,
      "Não conseguimos carregar essa lição",
      "A lição existe, mas o conteúdo dela não chegou até aqui. Quase sempre é a conexão: recarregar a página costuma resolver.",
      [
