...
function fonte(tamanho: number, peso = 400): string {
...
function textoEspacado(
  ...
  const larguraTotal =
    caracteres.reduce((soma, caractere) => soma + ctx.measureText(caractere).width, 0) +
...
function desenharPassaro(ctx: CanvasRenderingContext2D, cx: number, cy: number, escala: number): void {
  ctx.save();
  ctx.translate(cx, cy);
...
export function desenharCertificado(dados: DadosCertificado): HTMLCanvasElement {
  ...
  const desenharCanto = (x: number, y: number, sx: number, sy: number) => {
    ctx.fillStyle = ROSA;
    ctx.beginPath();
    ctx.moveTo(x, y);
  ...
  ctx.fillStyle = AZUL;
  textoCentradoAjustado(
    ctx,
    `“Atividades de ${dados.materia} — ${dados.ano}º Ano”`,
    880,
    62,
    700,
    LARGURA - 360
  ...
  ctx.font = fonte(34, 700);
  ctx.fillText(
    `${dados.acertos} de ${dados.total} atividades corretas · ${percentual}% de aproveitamento`,
    LARGURA / 2,
    1074
  );
...
export function nomeArquivoCertificado(nome: string, materia: string): string {
...
export function baixarCertificado(dados: DadosCertificado): void {
