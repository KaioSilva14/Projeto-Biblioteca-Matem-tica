...
import type { Certificado, CertificadoAno } from "./tipos.js";
 ...
 */
export function desenharCertificadoAno(dados: CertificadoAno): HTMLCanvasElement {
  const canvas = document.createElement("canvas");
  canvas.width = LARGURA;
  canvas.height = ALTURA;
  const ctx = canvas.getContext("2d");
  ...
  ctx.font = fonte(34, 400);
  ctx.fillText(
    `concluiu as ${dados.materias.length} matérias de Matemática do`,
    LARGURA / 2,
    642
  );
  ...
  ctx.font = fonte(26, 400, MONO);
  ctx.fillText(
    `${dados.licoes} lições · ${dados.questoes} questões · ${dados.acertosDePrimeira} acertadas de primeira · ${pct}%`,
    LARGURA / 2,
    866
  );
 ...
 */
function desenharMaterias(ctx: CanvasRenderingContext2D, materias: string[], topo: number): void {
  const COLUNAS = 3;
  ...
  for (let c = 0; c < COLUNAS; c++) colunas.push(materias.slice(c * linhas, (c + 1) * linhas));

  const larguras = colunas.map(
    (col) => RECUO_MARCA + Math.max(0, ...col.map((t) => ctx.measureText(t).width))
  );
...
function baixarCanvas(canvas: HTMLCanvasElement, arquivo: string): void {
  canvas.toBlob((blob) => {
    if (!blob) return;
...
export function baixarCertificadoAno(dados: CertificadoAno): void {
