...
function criarSvg(largura, altura, descricao) {
    const svg = el("svg", {
...
function plural(quantidade, singular, pluralPalavra) {
    return quantidade === 1 ? singular : pluralPalavra;
}
// ---------- Geradores ----------
/** Roda/pizza: círculo dividido em "partes" fatias iguais, "pintadas" delas destacadas. */
function desenharCirculo(partes, pintadas) {
    if (!Number.isInteger(partes) || partes < 2 || partes > 24)
        return null;
    if (pintadas < 0 || pintadas > partes)
        return null;
    const tamanho = 200;
    const centro = tamanho / 2;
    const raio = 84;
    const svg = criarSvg(tamanho, tamanho, `Círculo dividido em ${partes} ${plural(partes, "parte igual", "partes iguais")}, com ${pintadas} ${plural(pintadas, "parte pintada", "partes pintadas")}.`);
...
function desenharBarra(partes, pintadas) {
    if (!Number.isInteger(partes) || partes < 1 || partes > 20)
        return null;
    if (pintadas < 0 || pintadas > partes)
        return null;
    const largura = 320;
    ...
    const svg = criarSvg(largura, altura, `Barra dividida em ${partes} ${plural(partes, "parte igual", "partes iguais")}, com ${pintadas} ${plural(pintadas, "parte pintada", "partes pintadas")}.`);
    for (let i = 0; i < partes; i++) {
...
function desenharGrade(colunas, linhas, pintadas) {
    if (!Number.isInteger(colunas) || !Number.isInteger(linhas))
        return null;
    if (colunas < 1 || linhas < 1 || colunas > 12 || linhas > 12)
        return null;
    const total = colunas * linhas;
    if (pintadas < 0 || pintadas > total)
        return null;
    const lado = 36;
    const margem = 4;
    const largura = colunas * lado + margem * 2;
    const altura = linhas * lado + margem * 2;
    const svg = criarSvg(largura, altura, `Retângulo dividido em ${total} ${plural(total, "parte igual", "partes iguais")} (${colunas} por ${linhas}), com ${pintadas} ${plural(pintadas, "parte pintada", "partes pintadas")}.`);
...
function desenharReta(denominador, marcado) {
    if (!Number.isInteger(denominador) || denominador < 1 || denominador > 16)
        return null;
    if (marcado < 0 || marcado > denominador)
        return null;
    const largura = 340;
    const altura = 92;
    const margem = 26;
    ...
    const svg = criarSvg(largura, altura, `Reta numérica de 0 a 1 dividida em ${denominador} ${plural(denominador, "parte", "partes")}, com o ponto ${marcado}/${denominador} destacado.`);
...
function desenharConjunto(total, pintados) {
    if (!Number.isInteger(total) || total < 1 || total > 40)
        return null;
    if (pintados < 0 || pintados > total)
        return null;
    ...
    const svg = criarSvg(largura, altura, `Conjunto de ${total} objetos, com ${pintados} ${plural(pintados, "destacado", "destacados")}.`);
    for (let indice = 0; indice < total; indice++) {
 ...
 */
export function criarIlustracao(chave) {
...
export function renderIlustracao(chave) {
...
//# sourceMappingURL=ilustracoes.js.map