...
export function renderImagem(indice, chave, classe = "figura") {
    if (!chave)
        return null;
 ...
 */
export function renderPasso(indice, passo, numero) {
...
export function renderResolvido(indice, licao) {
...
export function renderQuestao(indice, questao, posicao, total) {
...
export function renderAcerto(mensagem) {
 ...
 */
export function renderErro(diagnostico, dica, primeiraVez) {
...
export function renderResolucao(indice, questao, resposta) {
...
export function renderProgressoLicao(feitas, total) {
...
export function renderCertificado(opcoes) {
    ...
    bloco.appendChild(el("p", {
        ...
        texto: "O certificado fica salvo só neste navegador, nesta máquina — não há cadastro nem servidor guardando nada. Se você limpar os dados do navegador, ele some. Baixe o arquivo para não depender disso.",
    }));
    atualizar();
    return bloco;
}
