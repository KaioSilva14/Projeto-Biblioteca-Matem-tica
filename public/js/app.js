...
import { lerLicao, registrarResposta, reiniciarLicao, reiniciarCurso, lerCurso, proximaLicao, resumirCurso, lerCertificado, emitirCertificado, listarCertificados, resumirAno, lerCertificadoAno, emitirCertificadoAno, removerCertificadoAno, listarCertificadosAno, } from "./progresso.js";
...
function initMenu() {
    const botao = qs("[data-menu]");
    const lista = qs("[data-menu-lista]");
    if (!botao || !lista)
        return;
...
async function iniciarHome() {
    ...
    const areaCert = qs("[data-certificados]");
    if (areaCert) {
...
// ---------- Página de ano ----------
async function iniciarAno() {
    const alvo = qs("[data-cursos]");
    if (!alvo)
        return;
    const catalogo = await json(CATALOGO);
    ...
    const nota = qs("[data-nota-embreve]");
    if (nota)
        nota.hidden = ano.cursos.every((c) => c.disponivel);
    limpar(alvo);
    for (const item of ano.cursos) {
        ...
        if (disponivel)
            linha.setAttribute("href", `/curso.html?c=${item.id}`);
...
function montarBlocoDoAno(ano) {
    ...
    const desenhar = () => {
        ...
        const nomeSugerido = salvo?.nome ?? listarCertificados().find((c) => c.nome.trim() !== "")?.nome ?? "";
        const montar = (nome) => ({
            ano: ano.ano,
...
async function iniciarCurso() {
    ...
    const desenhar = () => {
        ...
        if (resumo.concluido) {
            emitirCertificado(montarCertificado(curso, salvo?.nome ?? "", totalQuestoes, resumo.acertosDePrimeira, salvo));
        }
        const areaCert = el("div", { atributos: { id: "certificado" } });
        areaCert.appendChild(renderCertificado({
            concluido: resumo.concluido,
            ...
            desenhar: (nome) => desenharCertificado(montarCertificado(curso, nome, totalQuestoes, resumo.acertosDePrimeira, salvo)),
            aoBaixar: (nome) => baixarCertificado(montarCertificado(curso, nome, totalQuestoes, resumo.acertosDePrimeira, salvo)),
        }));
        ...
        if (curso.videos.length > 0) {
            ...
            secaoVideos.appendChild(el("p", {
                classe: "t-md cor-body leitura curso__nota-videos",
                texto: "As lições acima já ensinam a matéria inteira — estes vídeos são para quem quer ouvir a mesma ideia explicada por outra pessoa, ou ver mais exemplos resolvidos. Cada um traz uma nota dizendo para quando ele serve.",
            }));
...
function montarCertificado(curso, nome, questoes, acertos, anterior) {
    return {
        cursoId: curso.id,
...
function montarLicao(alvo, curso, licao, imagens) {
    ...
    function iniciarQuestao(posicao) {
    ...
    function montarQuestao(questao, posicao) {
        const caixa = el("div");
        ...
        const seguir = (acertouDePrimeira) => {
        ...
        btnVerificar.addEventListener("click", () => {
            ...
            if (veredito.certo) {
                montada.travar();
                limpar(acoes);
                retorno.appendChild(renderAcerto(tentativas === 1
...
function montarFim(curso, licao, progresso) {
    ...
    acoes.appendChild(el("a", { classe: "botao botao--nu", texto: "Todas as lições", atributos: { href: `/curso.html?c=${curso.id}` } }));
